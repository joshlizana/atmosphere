...
## 2026-04-24T19:30Z — addendum
- agent: log/01KQ02TDPA51RA7TYM67GSPXPT
- refs: [[files/JDL-66/scripts/init-sh]], [[files/JDL-66/scripts/up-sh]], [[.claude/context/operations]]

Forge modified `Makefile` at commit `8b15457` on branch `JDL-66-m1-shared-infrastructure-online`. Two lines changed inside the `up` target block: the `##` doc-comment was rewritten to describe the two-stage chain ("Pre-flight + first-boot init (init.sh), then tier-by-tier service bring-up with health-gating (up.sh).") and the recipe line changed from `./scripts/up.sh` to `./scripts/init.sh && ./scripts/up.sh`. Diff is `+2/-2`; no other lines in the file were touched. ^p011

The `&&` is load-bearing — it ensures that if `init.sh` fails (missing Docker Compose v2, wrong cwd, `.env` generation failure, etc.) the shell short-circuits and `up.sh` never runs. The platform never attempts service bring-up against an unvalidated environment or a half-generated `.env`. The pre-split recipe recorded in `^p002` above (`./scripts/up.sh` as the sole invocation) is now superseded by the two-stage chain; the split itself is the decision recorded in `[[decisions/JDL-66/20260424T1520Z-split-bring-up-init-and-up]]` (committed as `6f52a26`), which carved first-boot preflight and `.env` materialization out of `up.sh` and into the new `scripts/init.sh` so each script has one responsibility. ^p012

Preserved verbatim: the `.PHONY: up down nuke` declaration, the `down` target (`docker compose down`) and its doc-comment, the `nuke` target (`docker compose down -v --remove-orphans || true && rm -rf ./data && rm -f .env`) and its doc-comment, every header comment block (including the GNU-Make tab-indentation convention note), and every tab indentation on every recipe line. The `up` target itself retains its tab indentation, its preceding blank line, and its position as the first target — only the two lines inside its block change. ^p013

`make -n up` confirms the recipe prints as `./scripts/init.sh && ./scripts/up.sh` — the chain is correct and the shell short-circuit semantics are in effect. The file remains non-executable (Makefiles are read by `make`, not executed as scripts). The three-target surface (`up`, `down`, `nuke`) documented in `^p001` is unchanged — this modification is a recipe-level refinement of `up`, not an expansion of the target vocabulary. ^p014
